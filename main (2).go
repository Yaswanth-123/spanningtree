...
type Graph struct {
    vexs   []string                // fixed-point collection
    vexnum int                     //Fixed-point quantity
    edgnum int                     //Number of edges
    matrix [MAX_SIZE][MAX_SIZE]int //Adjacency matrix
}
...
func prim(gg *Graph, start int) {
    ...
    for i := 0; i < gg.vexnum; i++ {
        ...
        fmt.Println(gg.vexs[weights[next][0]], gg.vexs[next], "Weights", weights[next][1])
        sum += weights[next][1]
